...
class XLSXParser(BaseParser):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None, enable_vlm: bool = True):
        ...
        self.config.setdefault("max_rows_per_segment", 50)
    ...
    def _process_sheet_content(self, sheet, sheet_name: str, sheet_idx: int, start_idx: int, visual_elements: List[VisualElement]) -> List[Segment]:
        ...
        try:
            ...
            max_rows_per_segment = self.config.get("max_rows_per_segment", 50)
            
            for row_start in range(1, max_row + 1, max_rows_per_segment):
                ...
                if range_content:
                    segment = Segment(
                        ...
                        segment_type="data_range",
                        page_number=sheet_idx + 1,
                        visual_references=sheet_visual_refs,
                        metadata={
                            "sheet_name": sheet_name,
                            "row_start": row_start,
                            "row_end": row_end,
    ...
    def _build_document_content(self, segments: List[Segment], visual_elements: List[VisualElement]) -> str:
        ...
        for segment in segments:
            if segment.segment_type == "sheet_header":
