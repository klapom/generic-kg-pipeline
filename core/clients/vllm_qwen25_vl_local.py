"""
Local vLLM Qwen2.5-VL Client

High-performance visual analysis using local vLLM Qwen2.5-VL model.
Supports batch image processing and multimodal understanding.
"""
...
class VLLMQwen25VLClient(BaseVLLMClient):
    """
    Local vLLM Qwen2.5-VL Client
    
    Provides high-performance visual analysis using local vLLM model with:
    - Batch image processing
    - Chart and diagram analysis
    - OCR capabilities
    - Visual element type detection
    """
    ...
    def __init__(
        ...
        # Create model configuration optimized for Qwen2.5-VL
        ...
        logger.info(f"Initialized VLLMQwen25VLClient with batch_size={batch_size}")
    
    def get_warmup_prompts(self) -> List[str]:
        """Qwen2.5-VL specific warmup prompts"""
        return [
    ...
    def analyze_visual(
        ...
        try:
            ...
            logger.info("🔧 Ensuring Qwen2.5-VL model is loaded in LOCAL vLLM...")
            if not self.ensure_model_loaded():
                raise RuntimeError("Failed to load Qwen2.5-VL model")
            logger.info("✅ Qwen2.5-VL model ready in LOCAL vLLM")
            ...
            logger.info(f"🎨 LOCAL vLLM Qwen2.5-VL: Analyzing visual element (focus: {analysis_focus})")
            ...
            if not result.success:
                raise RuntimeError(f"Qwen2.5-VL inference failed: {result.error_message}")
            
            if not result.outputs:
                raise RuntimeError("No output from Qwen2.5-VL model")
    ...
    def analyze_visuals_batch(
        ...
        if not self.ensure_model_loaded():
            logger.error("Failed to load Qwen2.5-VL model for batch processing")
            return [VisualAnalysisResult(
