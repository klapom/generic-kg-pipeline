...
⚠️ DEPRECATED (16.07.2025) ⚠️
This service-based implementation is outdated and not functional.
It expects a structured API endpoint that does not exist.

Please use the local implementation instead:
- core/clients/vllm_smoldocling_local.py
- Used in: core/parsers/implementations/pdf/hybrid_pdf_parser.py
- Used in: scripts/process_documents_debug_enhanced.py

The local implementation includes:
- Proper DocTags parsing
- Coordinate prefix cleaning
- Complex layout detection
- Deduplication logic
- Robust error handling

This service implementation is kept for reference only.
"""
...
from core.parsers import Document, Segment, DocumentMetadata, DocumentType
...
class VLLMSmolDoclingClient(BaseModelClient[SmolDoclingRequest, SmolDoclingResult, SmolDoclingConfig], 
    ...
    """
    ⚠️ DEPRECATED (16.07.2025) - Use vllm_smoldocling_local.VLLMSmolDoclingClient instead ⚠️
    ...
    IMPORTANT: This implementation expects a service API that does not exist.
    The actual vLLM service returns raw SmolDocling DocTags output which this
    implementation cannot parse. Use the local implementation which includes
    proper DocTags parsing.
    """
    ...
    def __init__(self, config: Optional[SmolDoclingConfig] = None):
        ...
        logger.warning(
            "⚠️ DEPRECATED (16.07.2025): VLLMSmolDoclingClient (service) is deprecated. "
            "Use vllm_smoldocling_local.VLLMSmolDoclingClient instead. "
            "See core/clients/vllm_smoldocling_local.py"
        )
